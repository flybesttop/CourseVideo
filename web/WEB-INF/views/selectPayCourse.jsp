            ...
            <section id="content">
                ...
                <div class="container">
                    <div id="nav-tree-c" class="breadcrumbs nav-tree-c">
                        <c:choose>
                            <c:when test="${bestObjname==null&&objName==null}">
                                <p><a href="#">主页.</a> \\ 所有课程. \\ <span>全部.</span> <span></span></p>
                            </c:when>
                            <c:when test="${bestObjname!=null&&objName==null}">
                                <p><a href="#">主页.</a> \\ 所有课程. \\ <span>${bestObjname}.</span> <span></span></p>
                            </c:when>
                            <c:otherwise>
                                <p><a href="#">主页.</a> \\ 所有课程. \\ <span>${bestObjname}.</span> \\ <span>${objName}.</span></p>
                            </c:otherwise>
                        </c:choose>

                    </div>
                    <div class="select-nav">
                        <div class="select-nav-t">
                            <h5 class="line">
                                <span>付费课程.</span>
                            </h5>
                        </div>
                        <div class="breadcrumbs select-nav-row" id="select-box-1">
                            <span>方向:</span>
                            <ul>
                                <c:choose>
                                    <c:when test="${bestObjname==null}">
                                        <li class="selected-item"><a href="selectPayCourse">全部</a></li>
                                    </c:when>
                                    <c:otherwise>
                                        <li><a href="selectPayCourse">全部</a></li>
                                    </c:otherwise>
                                </c:choose>
                                <%--<li class="selected-item"><a href="#" >全部</a></li>--%>
                                <c:forEach var="bestobjs" items="${bestObjs}">
                                    <c:choose>
                                        <c:when test="${bestObjname==bestobjs.acRelationshipsItemname}">
                                            <li class="selected-item"><a href="selectPayCourse?bestObjId=${bestobjs.id}"
                                                                         target="_self"
                                                                         bestObjId="${bestobjs.id}">${bestobjs.acRelationshipsItemname}</a>
                                            </li>
                                        </c:when>
                                        <c:otherwise>
                                            <li><a href="selectPayCourse?bestObjId=${bestobjs.id}" target="_self"
                                                   bestObjId="${bestobjs.id}">${bestobjs.acRelationshipsItemname}</a>
                                            </li>
                                        </c:otherwise>
                                    </c:choose>
                        ...
                        <div class="select-nav-row" id="select-box-2">
                            <span>类型:</span>
                            <ul>
                                <c:choose>
                                    <c:when test="${objId==null}">
                                        <li class="selected-item"><a href="selectPayCourse">全部</a></li>
                                    </c:when>
                                    <c:otherwise>
                                        <li><a href="selectPayCourse?bestObjId=${bestObjId}">全部</a></li>
                                    </c:otherwise>
                                </c:choose>
                                <%--<li class="selected-item"><a href="select?bestObjId=${bestObjId}">全部</a></li>--%>

                                <c:forEach var="objs" items="${relationshipslevel2}">
                                    <c:choose>
                                        <c:when test="${objId==objs.acRelationshipsItemid}">
                                            <li class="selected-item"><a
                                                    href="selectPayCourse?bestObjId=${objs.acRelationshipsParendid}&objs=${objs.acRelationshipsItemid}">${objs.acRelationshipsItemname}</a>
                                            </li>
                                        </c:when>
                                        <c:otherwise>
                                            <li>
                                                <a href="selectPayCourse?bestObjId=${objs.acRelationshipsParendid}&objs=${objs.acRelationshipsItemid}">${objs.acRelationshipsItemname}</a>
                                            </li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                    ...
                    <div class="content">
                        ...
                        <div class="relatednews search-cloumn-4">
                            <h5 class="line"><span>搜索结果.</span></h5>
                            <ul>
                                <c:forEach var="course" items="${courses}">
                                    <li>
                                        ...
                                        <span class="playData"><i
                                                class="icon-play"></i>选购量：<span>${course.acCoursePicknum}</span></span>
                                    </li>
                                </c:forEach>
                            </ul>
                        </div>
                        <ul id="pageBox"></ul>
                        <form action="selectPayCourse" method="get">
                            <c:if test="${bestObjId!=null}">
                                <input type="hidden" name="bestObjId" value="${bestObjId}"/>
                            </c:if>
                            <c:if test="${objId!=null}">
                                <input type="hidden" name="objs" value="${objId}"/>
                            </c:if>
